...
import type { EmitNoteInput } from '@/api/notes'
...
export interface EmitNotePageConfig {
  title: string
  subtitle: string
  noteTypeName: string
  motivoLabel: string
  motivoOptions: MotivoOption[]
  emitFn: (input: EmitNoteInput) => Promise<EmitResult>
...
interface WizardState {
  // Step 1: Reference + Motivo + General data
...
const STEPS = [
  { number: 1, label: 'Referencia y Datos' },
...
function getSeriesPrefix(refDocType: string): string {
  return refDocType === '03' ? 'B' : 'F'
}

function initialState(refDocId?: string, refDocType?: string): WizardState {
  const docType = refDocType === '03' ? '03' : '01'
  const prefix = getSeriesPrefix(docType)
  return {
...
function validateStep1(s: WizardState): string | null {
  if (!s.referenceDocumentId.trim()) return 'El documento de referencia es obligatorio'
  if (!s.responseCode) return 'Debe seleccionar un motivo'
...
export default function EmitNotePage({ config }: { config: EmitNotePageConfig }) {
  ...
  useEffect(() => {
    ...
    if (ref || type) {
      setForm((prev) => ({
        ...prev,
        referenceDocumentId: ref ?? prev.referenceDocumentId,
        referenceDocumentType: type === '03' ? '03' : '01',
        series: `${getSeriesPrefix(type === '03' ? '03' : '01')}001`,
      }))
    }
  }, [searchParams])
  ...
  // Get motivo description from code
  const motivoDescription = useMemo(() => {
    const option = config.motivoOptions.find((o) => o.code === form.responseCode)
    return option?.label ?? ''
  }, [config.motivoOptions, form.responseCode])
  ...
  function handleRefDocTypeChange(newType: string) {
    const prefix = getSeriesPrefix(newType)
    setForm((prev) => ({
      ...prev,
      referenceDocumentType: newType,
      series: `${prefix}${prev.series.slice(1)}`,
    }))
    setStepError(null)
  }

  function handleResponseCodeChange(code: string) {
    const option = config.motivoOptions.find((o) => o.code === code)
    setForm((prev) => ({
      ...prev,
      responseCode: code,
      responseDescription: option?.label ?? '',
    }))
    setStepError(null)
  }
  ...
  function handleEmit() {
    ...
    const payload: EmitNoteInput = {
      ...
      referenceDocumentId: form.referenceDocumentId.trim(),
  ...
  // ---------------------------------------------------------------------------
  // Render: Step 1 - Referencia + Datos Generales + Cliente
  // ---------------------------------------------------------------------------
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="referenceDocumentId">Documento de Referencia *</Label>
                <Input
                  id="referenceDocumentId"
                  value={form.referenceDocumentId}
                  onChange={(e) => updateField('referenceDocumentId', e.target.value.toUpperCase())}
                  placeholder="F001-1"
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="referenceDocumentType">Tipo de Documento de Referencia *</Label>
                <select
            ...
            <div className="space-y-2">
              <Label htmlFor="responseCode">{config.motivoLabel} *</Label>
              <select
                id="responseCode"
                className={selectClassName}
                value={form.responseCode}
                onChange={(e) => handleResponseCodeChange(e.target.value)}
              >
                <option value="">-- Seleccione un motivo --</option>
                {config.motivoOptions.map((m) => (
  ...
  function renderStep3() {
    ...
    const refDocTypeLabel = REFERENCE_DOC_TYPES.find((t) => t.code === form.referenceDocumentType)?.label ?? form.referenceDocumentType
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
              <div>
                ...
                <p className="font-medium font-mono">{form.referenceDocumentId}</p>
              </div>
              <div>
                <span className="text-muted-foreground">Tipo:</span>
                <p className="font-medium">{form.referenceDocumentType} - {refDocTypeLabel}</p>
              </div>
              <div className="col-span-2">
                <span className="text-muted-foreground">Motivo:</span>
                <p className="font-medium">{form.responseCode} - {motivoDescription}</p>
              </div>
            </div>
          </div>
