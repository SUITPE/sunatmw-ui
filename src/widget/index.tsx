...
// ---------------------------------------------------------------------------
// <suit-widget> Web Component
//
// Attributes:
//   api-key  (required) - API key for authentication
//   api-url  (optional) - Base URL of the SUIT API
...
class SuitWidget extends HTMLElement {
  ...
  private render(): void {
    ...
    if (!apiKey) {
      this.shadow.innerHTML = `
        <div style="padding:16px;color:#dc2626;font-family:sans-serif;font-size:14px;border:1px solid #dc2626;border-radius:6px;">
          Error: el atributo <code>api-key</code> es obligatorio en &lt;suit-widget&gt;.
        </div>
      `
      return
    }
    ...
    if (!this.root) {
      ...
      mountPoint.setAttribute('id', 'suit-widget-mount')
...
if (!customElements.get('suit-widget')) {
  customElements.define('suit-widget', SuitWidget)
}
